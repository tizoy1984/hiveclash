...
export default function App() {
  ...
  const renderDashboard = () => (
    <div className="min-h-screen bg-slate-50 dark:bg-black text-slate-900 dark:text-gray-100 flex flex-col transition-colors duration-500">
      ...
      <main className="flex-1 max-w-7xl mx-auto p-6 w-full space-y-16 mt-8">
        ...
        {/* Live Section (RESTORED WITH PRIZE BADGES) */}
        <section>
          <div className="flex items-center justify-between mb-8 border-b dark:border-white/10 pb-4">
            ...
            <span className="text-cyan-400 font-bold bg-cyan-400/10 px-4 py-1 rounded-full text-sm">Active Rewards</span>
          </div>
            ...
            {INITIAL_GAMES.filter(g => g.status === 'live').map(game => (
              <div key={game.id} className="bg-zinc-900/50 rounded-3xl overflow-hidden border border-white/5 hover:border-cyan-500 transition-all group">
                <div className="relative h-56">
                  ...
                  <div className="absolute top-4 right-4 bg-rose-500 px-3 py-1 rounded-full text-xs font-black animate-pulse shadow-lg shadow-rose-500/50">LIVE</div>
        ...
        {/* Upcoming Section (RESTORED WITH AVATAR STACKS) */}
        <section>
  ...
  const renderHostSetup = () => (
    <div className="min-h-screen bg-slate-50 dark:bg-black flex items-center justify-center p-6 transition-colors duration-500">
  ...
  const renderWaitingRoom = () => (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center p-10 relative overflow-hidden transition-all">
    ...
    return (
      <div className="min-h-screen bg-black text-white p-6 flex flex-col items-center justify-center relative transition-colors duration-500">
        ...
        <div className="grid grid-cols-2 gap-8 w-full max-w-6xl">
          {q.options.map((opt, i) => (
            <button key={i} onClick={() => setSelectedAnswer(i)} className={`${COLORS[i]} p-12 sm:p-20 rounded-3xl flex flex-col items-center justify-center transition-all ${selectedAnswer === i ? 'ring-8 ring-white scale-105' : 'opacity-80'}`}>
