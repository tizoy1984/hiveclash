...
const QUESTIONS = [
  ...
  { id: 3, text: "Which operation is used to store game data for free?", options: ["transfer", "vote", "custom_json", "delegate"], correct: 2, type: "multiple" }
];

const COLORS = [
  "bg-rose-500 hover:bg-rose-400 border-rose-600 shadow-lg dark:shadow-[0_0_20px_rgba(225,29,72,0.4)]",
  "bg-cyan-500 hover:bg-cyan-400 border-cyan-600 shadow-lg dark:shadow-[0_0_20px_rgba(6,182,212,0.4)]",
  "bg-amber-500 hover:bg-amber-400 border-amber-600 shadow-lg dark:shadow-[0_0_20px_rgba(245,158,11,0.4)]",
  "bg-emerald-500 hover:bg-emerald-400 border-emerald-600 shadow-lg dark:shadow-[0_0_20px_rgba(16,185,129,0.4)]"
];

const ICONS = [Triangle, Hexagon, Circle, Square];
const MOCK_USERS = ['hiveking', 'cryptogamer', 'block_master', 'web3_ninja', 'crypto_queen', 'hive_dev'];
...
const Confetti = () => (
  <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
    {[...Array(50)].map((_, i) => (
      <div key={i} className="absolute w-3 h-3 animate-fall" style={{ left: `${Math.random() * 100}vw`, top: `-5vh`, backgroundColor: ['#06b6d4', '#ec4899', '#8b5cf6', '#10b981'][Math.floor(Math.random() * 4)], animationDuration: `${Math.random() * 2 + 2}s` }} />
    ))}
  ...
  const renderDashboard = () => (
    <div className="min-h-screen bg-slate-50 dark:bg-black text-slate-900 dark:text-gray-100 flex flex-col transition-colors duration-500">
      <header className="bg-white/80 dark:bg-black/40 backdrop-blur-xl border-b dark:border-white/10 p-4 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-6">
            {!username ? (
              <button onClick={() => setIsLoginModalOpen(true)} className="bg-white/5 px-6 py-2 rounded-full font-bold border dark:border-white/10 flex items-center"><Key size={18} className="mr-2 text-fuchsia-400" /> Login</button>
            ) : (
      ...
      <main className="flex-1 max-w-7xl mx-auto p-6 w-full space-y-16 mt-8">
        {/* Hero */}
        <div className="relative rounded-3xl p-12 text-white overflow-hidden border dark:border-white/10 shadow-2xl bg-gradient-to-r from-zinc-900 to-black">
          ...
          <Trophy size={140} className="absolute right-12 top-1/2 -translate-y-1/2 text-fuchsia-500/20 rotate-12" />
        </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex -space-x-2">
                    {game.interested.map(u => (
                      <img key={u} src={`https://images.hive.blog/u/${u}/avatar`} className="w-8 h-8 rounded-full border-2 border-zinc-900 shadow-md" title={`@${u}`} />
                    ))}
                    <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center text-xs font-bold border-2 border-zinc-900">+{game.interested.length}</div>
                  </div>
       ...
       <div className="bg-white dark:bg-zinc-900 p-10 rounded-3xl w-full max-w-lg border border-gray-200 dark:border-white/10 shadow-2xl relative">
          ...
          <div className="space-y-6">
            <div>
              ...
              <input className="w-full p-4 rounded-xl bg-gray-50 dark:bg-black border dark:border-white/10 font-bold dark:text-white shadow-inner" value={hostGameTitle} onChange={e => setHostGameTitle(e.target.value)} placeholder="e.g. Hive Expert Quiz" />
            </div>
            <div>
    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center p-10 relative overflow-hidden">
       ...
       <div className="z-10 w-full max-w-6xl">
          <div className="flex items-center mb-8 border-b border-white/10 pb-4">
             ...
             <h3 className="text-3xl font-black">Challengers ({players.length})</h3>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
            {players.length === 0 ? (
               <div className="col-span-full py-20 text-center text-2xl font-bold text-gray-500 animate-pulse">Waiting for players to enter the PIN...</div>
            ) : (
              players.map((p, i) => (
  ...
  const renderLeaderboard = () => (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-10 relative">
       ...
       <div className="w-full max-w-2xl space-y-4">
          <div className="flex justify-between items-center bg-zinc-900 p-8 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden">
             <div className="absolute top-0 left-0 w-2 h-full bg-amber-500"></div>
             <span className="text-3xl font-black text-gray-400 mr-4">#1</span>
             <span className="text-4xl font-black flex-1">@{username || 'Player'}</span>
  ...
  return (
    <div className={isDarkMode ? "dark" : ""}>
      <div className="font-sans bg-slate-50 dark:bg-black min-h-screen transition-colors duration-500">
        ...
        {/* Indexer Sidebar Logs */}
        <div className="hidden md:block fixed bottom-6 right-6 w-80 bg-black/80 backdrop-blur-xl p-5 rounded-2xl border border-white/10 z-50 pointer-events-none transition-all">
          <div className="flex items-center space-x-2 text-white font-bold mb-3 border-b border-white/10 pb-2">
            ...
            <span className="text-xs uppercase tracking-widest">Hive Indexer Activity</span>
          </div>
          <div className="space-y-1.5 text-[10px] text-gray-400 font-mono">
            {logs.length === 0 ? <span className="italic opacity-50">Monitoring network...</span> : logs.map((log, i) => <div key={i}>{log}</div>)}
          </div>
        </div>

        {notification && (
          <div className="fixed top-24 left-1/2 transform -translate-x-1/2 bg-rose-600 text-white px-6 py-3 rounded-full z-50 animate-bounce font-bold border border-rose-400 shadow-lg">{notification}</div>
        )}
      </div>
    </div>
  );
