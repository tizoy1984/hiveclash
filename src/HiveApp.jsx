...
export default function App() {
  ...
  const handleKeychainLogin = () => {
    ...
    setTimeout(() => {
      ...
      addLog(`@${loginInput.toLowerCase()} signed in via Keychain`);
    }, 1200);
  };

  const startHosting = async () => {
    if (!username.trim()) { setIsLoginModalOpen(true); return; }
    if (!hostGameTitle.trim()) { triggerNotification("Enter a game title."); return; }
  ...
  const handleAnswer = (index) => {
    ...
    addLog(`Choice ${index} broadcasted by @${username}`);
  };
      ...
      <header className="bg-white/80 dark:bg-black/40 backdrop-blur-xl border-b dark:border-white/10 p-4 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-6">
            {!username ? (
              <button onClick={() => setIsLoginModalOpen(true)} className="bg-white/5 px-6 py-2 rounded-full font-bold border dark:border-white/10 flex items-center shadow-sm">
      ...
      <main className="flex-1 max-w-7xl mx-auto p-6 w-full space-y-16 mt-8">
        <div className="relative rounded-3xl p-12 text-white overflow-hidden border dark:border-white/10 shadow-2xl bg-gradient-to-r from-zinc-900 to-black group transition-all">
          <div className="relative z-20">
            ...
            <p className="text-gray-400 text-xl font-medium max-w-lg">Active games are happening now. Join the lobby and compete for the prize pool.</p>
          </div>
            ...
            {INITIAL_GAMES.filter(g => g.status === 'live').map(game => (
              <div key={game.id} className="bg-zinc-900/50 rounded-3xl overflow-hidden border border-white/5 hover:border-cyan-500 transition-all group">
                ...
                <div className="p-8">
                  ...
                  <button onClick={() => joinLiveGame(game)} className="w-full mt-6 bg-cyan-600 py-4 rounded-xl font-black text-lg hover:bg-cyan-500 transition-all shadow-md">JOIN NOW</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* UPCOMING SECTION WITH MEMBERS JOINING */}
        <section>
          <div className="flex items-center mb-8 border-b dark:border-white/10 pb-4 justify-between">
       ...
       <div className="bg-white dark:bg-zinc-900 p-10 rounded-3xl w-full max-w-lg border border-gray-200 dark:border-white/10 shadow-2xl relative">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input className="w-full p-4 rounded-xl bg-gray-50 dark:bg-black border dark:border-white/10 font-bold dark:text-white" type="number" value={hostFunding} onChange={e => setHostFunding(e.target.value)} />
            </div>
            
            {/* SCHEDULING TOGGLE POSSIBILITY */}
  ...
  const renderPlaying = () => {
    ...
    return (
      <div className="min-h-screen bg-black text-white p-6 flex flex-col items-center justify-center relative transition-colors duration-500">
        <div className="absolute top-10 flex items-center space-x-6 text-6xl font-black shadow-cyan-500/20 shadow-2xl">
        ...
        <h2 className="text-5xl sm:text-7xl font-black mb-16 text-center max-w-5xl leading-tight drop-shadow-lg">
          {viewMode === 'host' ? q.text : "Check the Host TV!"}
        </h2>
